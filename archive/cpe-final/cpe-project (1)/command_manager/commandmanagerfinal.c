...
void *handle_client(void *arg) {
    ...
    //sleep(30); // Simulate delay (30 seconds) for testing
    ...
    if (is_background) {
        ...
        int master_fd;
        // Create pseudo-terminal for command execution
        pid_t pid = forkpty(&master_fd, NULL, NULL, NULL);
        if (pid < 0) {
            perror("forkpty failed");
            strcpy(output, "Failed to start background command\n");
            total_output = strlen(output);
            write(client_socket, output, total_output); // Send error to client
            goto cleanup;
        }
        if (pid == 0) {
            ...
            char log_file[64];
            // Create log file path for output redirection
            snprintf(log_file, sizeof(log_file), "/tmp/bg_%d.log", getpid());
            char wrapped_command[BUFFER_SIZE * 2];
            // Redirect output to log file if not already redirected
        ...
        // Parent process: monitor initial output
        ...
        timeout.tv_sec = 1; // Wait 1 second for initial output
        timeout.tv_usec = 0;
        int activity = select(master_fd + 1, &fds, NULL, NULL, &timeout);
        if (activity > 0 && FD_ISSET(master_fd, &fds)) {
            // Read initial output from pseudo-terminal
        ...
        // Send process ID and log file path to client
        char pid_msg[128];
        char log_file[64];
        ...
        if (total_output > 0) {
            write(client_socket, output, total_output); // Send initial output
        }
