...
#define LONG_RESPONSE_TIMEOUT 310
...
static volatile int system_running = 1; // Flag to control program execution
int command_server_fd = -1; // File descriptor for command server

// Subsection: Client Management
int command_client_fds[MAX_CLIENTS];
int num_clients = 0;
...
// Section: Alarm Server
void broadcast_alarm(const char *message, const char *time_str) {
    if (!is_system_running()) return;
    char alarm_msg[BUFFER_SIZE];
    snprintf(alarm_msg, sizeof(alarm_msg), "\n[ALARM RECEIVED] %s: %s\n", time_str, message);

    pthread_mutex_lock(&clients_mutex);
    for (int i = 0; i < num_clients; i++) {
        if (send(command_client_fds[i], alarm_msg, strlen(alarm_msg), 0) < 0) {
            perror("Failed to send alarm to client");
            close(command_client_fds[i]);
            command_client_fds[i] = command_client_fds[num_clients - 1];
            num_clients--;
            i--;
        }
    }
...
enum MHD_Result alarm_handler(void *cls, struct MHD_Connection *connection,
                             const char *url, const char *method, const char *version,
                             const char *upload_data, size_t *upload_data_size, void **con_cls) {
    if (!is_system_running()) return MHD_YES;
    if (strcmp(method, "POST") != 0 || strcmp(url, "/alarm") != 0) {
        struct MHD_Response *response = MHD_create_response_from_buffer(0, "", MHD_RESPMEM_PERSISTENT);
        int ret = MHD_queue_response(connection, MHD_HTTP_METHOD_NOT_ALLOWED, response);
        MHD_destroy_response(response);
            ...
            if (message_end) {
                *message_end = '\0';
                ...
                if (time_str) {
                    ...
                    printf("\n[ALARM RECEIVED] %s: %s\n", time_str, message_start);
                    fflush(stdout);
                    broadcast_alarm(message_start, time_str);
                } else {
                    printf("\n[ALARM RECEIVED] %s\n", message_start);
                    fflush(stdout);
                    broadcast_alarm(message_start, "unknown_time");
                }
            }
        }
        free(data);
...
void *alarm_server_thread(void *arg) {
    if (!is_system_running()) return NULL;
    struct sockaddr_in addr = {0};
    addr.sin_family = AF_INET;
    addr.sin_addr.s_addr = inet_addr("127.0.0.1");
    addr.sin_port = htons(ALARM_PORT);
    ...
    printf("Alarm server running on 127.0.0.1:%d\n", ALARM_PORT);
...
// Section: Command Server
void *command_server_thread(void *arg) {
    if (!is_system_running()) return NULL;
    command_server_fd = socket(AF_INET, SOCK_STREAM, 0);
    if (command_server_fd < 0) {
        perror("Failed to create command server socket");
        pthread_exit(NULL);
    }
    ...
    int opt = 1;
    if (setsockopt(command_server_fd, SOL_SOCKET, SO_REUSEADDR, &opt, sizeof(opt)) < 0) {
        perror("setsockopt SO_REUSEADDR");
        close(command_server_fd);
        pthread_exit(NULL);
    }

    if (bind(command_server_fd, (struct sockaddr *)&addr, sizeof(addr)) < 0) {
        perror("Bind command server socket");
        close(command_server_fd);
        pthread_exit(NULL);
    }

    if (listen(command_server_fd, 10) < 0) {
        perror("Listen on command server socket");
        close(command_server_fd);
        pthread_exit(NULL);
    }

    while (is_system_running()) {
        int client_fd = accept(command_server_fd, NULL, NULL);
        if (client_fd < 0) {
            if (!is_system_running()) break;
            perror("Accept command connection");
            continue;
        }

        pthread_mutex_lock(&clients_mutex);
        if (num_clients < MAX_CLIENTS) {
            command_client_fds[num_clients++] = client_fd;
...
int main() {
    ...
    if (pthread_create(&command_thread, NULL, command_server_thread, NULL)) {
        perror("Failed to start command server thread");
        return 1;
    }
