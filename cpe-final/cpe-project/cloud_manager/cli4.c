...
#define EOF_TIMEOUT 1      // Timeout for server response after Ctrl+D
...
int main() {
    ...
    int sent_eof = 0; // Track if Ctrl+D was sent
    ...
    while (1) {
        ...
        if (ret == 0) {
            ...
            // Check for EOF response timeout
            if (sent_eof && difftime(now, last_activity) >= EOF_TIMEOUT) {
                printf("\n[Cloud Manager] No response after EOF, resetting.\n");
                restore_mode(&original);
                command_sent = 0;
                is_interactive = 0;
                sent_eof = 0;
        ...
        if (fds[1].revents & (POLLIN | POLLERR | POLLHUP)) {
            ...
            if (strstr(buffer, TERMINATION_MESSAGE)) {
                restore_mode(&original);
                command_sent = 0;
                is_interactive = 0;
                sent_eof = 0;
        ...
        if (fds[2].revents & POLLIN) {
            if (!command_sent) {
                ...
                // Check for Ctrl+D to signal EOF
                if (n == 1 && buffer[0] == CTRL_D && !sent_eof) {
                    // Send Ctrl+D byte to server
                    if (write(command_sock, buffer, 1) < 0) {
                        fprintf(stderr, "Failed to send Ctrl+D: %s\n", strerror(errno));
                        restore_mode(&original);
                        command_sent = 0;
                        is_interactive = 0;
                        sent_eof = 0;
